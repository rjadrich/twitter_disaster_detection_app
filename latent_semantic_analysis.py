...
class latent_semantic_indexer:  
    ...
    def make_dictionary_and_corpus(self, corpus_df, min_word_count = 1):
    ...
    #def sort_by_similarity(self, df, text):
    ...
    #    #generate LSI vector and index it against the whole corpus and apply similarity result to dataframe
    #    df["Similarity_0"] = [0.0 for num in range(len(df))]
    #    for text in texts:
    #        vector_lsi = self.make_lsi_vector(text)
    #        sims = self.index[vector_lsi]
    #        df["Similarity_1"] = sims
    #        df["Similarity"] = maximum(df["Similarity_0"], df["Similarity_1"])
    #        df["Similarity_0"] = df["Similarity_1"]
        
    #    df = df.drop('Similarity_0', 1)
    #    df = df.drop('Similarity_1', 1)
